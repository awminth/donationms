        ...
        while($row = mysqli_fetch_array($result)){
            ...
            $out.="<tr>
                ...
                <td class='text-center'>
					...
					";
					if($_SESSION["donationms_usertype"] == "Admin"){
                        $out.="
...
if($action == 'save'){
    ...
    if($stmt->execute()){
        save_log($_SESSION["donationms_username"]." သည် user (".$username.") အား အသစ်သွင်းသွားသည်။");
        echo 1;
    }else{
...
if($action == 'edit'){
    ...
    $sql = "UPDATE tbluser SET UserName=?, Password=?, UserType=?  
    WHERE AID=?";
    $stmt = $con->prepare($sql);
    $stmt->bind_param("sssi", $username, $password, $usertype, $aid);
    if($stmt->execute()){
        save_log($_SESSION["donationms_username"]." သည် user (".$username.") အား update သွားသည်။");
        echo 1;
    }else{
...
if($action == 'delete'){
    ...
    if($stmt->execute()){
        save_log($_SESSION["donationms_username"]." သည် user (".$username.") အား delete သွားသည်။");
        echo 1;
    }else{
...
if($action == "changeprofile"){
    $aid = $_SESSION['donationms_userid'];
    $password = $_POST["password"];
    $img = $_POST["img"];
    $sql = "update tbluser set Password='{$password}',Img='{$img}' where AID='{$aid}'";
    if(mysqli_query($con,$sql)){
        $_SESSION['donationms_userpassword'] = $password;
        $_SESSION['donationms_img'] = $img;
        save_log($_SESSION['donationms_username']." Password change သွားသည်");
        echo 1;
    }else{
        echo 0;
    }
